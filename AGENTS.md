...
| 导航 | `OPEN_DOUYIN_TAB` |
| Tab 转发 | `SYNC_WORKS` / `FETCH_FOLLOWING` / `FETCH_FAVORITES` / `CANCEL_LIKE` / `FETCH_COLLECTION` / `CANCEL_COLLECTION` / `FETCH_WORKS_PAGE` / `GET_SECURITY_STATUS` |
...
| 进度转发 | `SYNC_PROGRESS` / `FOLLOWING_PROGRESS` / `FAVORITES_PROGRESS` / `COLLECTION_PROGRESS` |

**长任务链路模式**：
- `sendToTab`：background 生成 `requestId`，向抖音标签页发消息，等待超时默认 300s（`FETCH_WORKS_PAGE` 60s，`CANCEL` 120s，`GET_SECURITY_STATUS` 5s）。`sendToTab` 内部 `.catch()` 处理 `withDouyinTab()` 极端异常路径。
...
`options/options.js` 顶层 `config` 常量（39 个键）：

| 分组 | 键 |
|---|---|
| 视频重试 | `VIDEO_RETRY_DELAYS` `[200,400,600]` / `VIDEO_RETRY_MAX` `3` / `VIDEO_RETRY_FALLBACK_DELAY` `1000` |
| 超时 | `FETCH_RETRY_DELAY` `1000` / `SYNC_TIMEOUT` `30000` / `TOAST_DURATION` `2000` / `VIDEO_FALLBACK_TIMEOUT` `5000` |
...
| 详情页 | `DETAIL_TITLE_MAX_LEN` `40` / `DOWNLOAD_MAX_RETRY` `1` |
...
| [SUGGESTIONS.md](./SUGGESTIONS.md) | 死代码与 Bug 修复记录 |
