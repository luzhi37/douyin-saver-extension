...
(function () {
  ...
  function forwardEvent(eventName, msgType, mapFn) {
    document.addEventListener(eventName, function (event) {
      const d = event.detail || {};
      chrome.runtime.sendMessage({ type: msgType, ...(mapFn ? mapFn(d) : d) }).catch(() => {});
    });
  }

  forwardEvent('DY_FOLLOWING_PROGRESS', 'FOLLOWING_PROGRESS', function (d) {
    return { collected: d.collected, hasMore: d.hasMore, total: d.total, requestId: d.requestId };
  });
  forwardEvent('DY_FAVORITES_PROGRESS', 'FAVORITES_PROGRESS', function (d) {
    return { collected: d.collected, unfollowedCount: d.unfollowedCount, hasMore: d.hasMore, total: d.total, requestId: d.requestId };
  });
  forwardEvent('DY_COLLECTION_PROGRESS', 'COLLECTION_PROGRESS', function (d) {
    return { collected: d.collected, unfollowedCount: d.unfollowedCount, hasMore: d.hasMore, total: d.total, requestId: d.requestId };
  });
  forwardEvent('DY_SYNC_WORKS_PROGRESS', 'SYNC_PROGRESS', function (d) {
    return { requestId: d.requestId, index: d.index, status: d.status, total: d.total, awemeId: d.awemeId };
  });
  ...
  function requestResponse(requestEvent, resultEvent, timeoutMs, buildDetail) {
    return function (message, _sender, sendResponse) {
      if (!message.requestId) { sendResponse({ ok: false, error: 'INVALID_REQUEST' }); return false; }
  ...
  chrome.runtime.onMessage.addListener((message, _sender, sendResponse) => {
    switch (message.type) {
      case 'SYNC_WORKS':
        return requestResponse('DY_SYNC_WORKS_REQUEST', 'DY_SYNC_WORKS_RESULT', 300000, function (msg) {
          return { awemeIds: msg.awemeIds };
        })(message, _sender, sendResponse);

      // 关注域 (tools 移植)
      case 'FETCH_FOLLOWING':
        return requestResponse('DY_FETCH_FOLLOWING_REQUEST', 'DY_FETCH_FOLLOWING_RESULT', 300000, function (msg) {
          return { secUid: msg.secUid };
        })(message, _sender, sendResponse);
      ...
      case 'FETCH_FAVORITES':
        return requestResponse('DY_FETCH_FAVORITES_REQUEST', 'DY_FETCH_FAVORITES_RESULT', 300000, function (msg) {
          return { secUid: msg.secUid };
        })(message, _sender, sendResponse);

      case 'CANCEL_LIKE':
        return requestResponse('DY_CANCEL_LIKE_REQUEST', 'DY_CANCEL_LIKE_COMPLETE', 120000, function (msg) {
          return { awemeIds: msg.awemeIds };
        })(message, _sender, sendResponse);

      case 'FETCH_COLLECTION':
        return requestResponse('DY_FETCH_COLLECTION_REQUEST', 'DY_FETCH_COLLECTION_RESULT', 300000, function (msg) {
          return { cursor: msg.cursor || 0 };
        })(message, _sender, sendResponse);

      case 'CANCEL_COLLECTION':
        return requestResponse('DY_CANCEL_COLLECTION_REQUEST', 'DY_CANCEL_COLLECTION_COMPLETE', 120000, function (msg) {
          return { awemeIds: msg.awemeIds };
        })(message, _sender, sendResponse);
  ...
  function fetchDetailByAwemeId(awemeId, timeoutMs = 5000) {
    return new Promise((resolve) => {
      ...
      document.dispatchEvent(new CustomEvent('DY_FETCH_DETAIL_REQUEST', { detail: { awemeId, requestId } }));
    });
  }
